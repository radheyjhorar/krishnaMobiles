...
const CustomerPaymentList = () => {
  ...
  const getCustomerPayment = async () => {
    let result = await fetch('http://localhost:5000/customer-payment', { method: 'Get' });
  ...
  const deletePayment = async (id) => {
    let result = await fetch(`http://localhost:5000/customer-payment/${id}`, {
      method: 'Delete',
    });
    result = await result.json(); 
    if (result) {
              ...
              return (
                <tr key={index}>
                  ...
                  <td className='p-0'>
                    ...
                    <Link to={"/update-customer-payment/" + item.id}>
